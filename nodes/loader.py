...
HF_MODELS = {
    ...
    # INT4 PATCH: pre-quantized NF4 checkpoint (export_nf4.py was used to create it).
    # Uses the same base weights but quantized once and saved — faster startup than
    # on-the-fly nf4 which re-quantizes every time. See tools/export_nf4.py.
    "s2-pro-bnb-nf4-prequant": {
        ...
        "base_model": "s2-pro-bnb-nf4-prequant",   # stored in its own folder after export
        "prequant": True,
    },
}
...
def resolve_bnb_mode(model_name: str) -> str | None:
    """
    Return 'int8', 'nf4', 'nf4-prequant', or None based on model_name.
    BNB models are: s2-pro-bnb-int8, s2-pro-bnb-nf4, s2-pro-bnb-nf4-prequant
    
    INT4 PATCH: Added 'nf4-prequant' mode for pre-quantized checkpoints created
    with tools/export_nf4.py. These load faster because no on-the-fly quantization
    is needed at startup.
    """
    name_lower = model_name.lower()
    if "bnb-int8" in name_lower or "bnb_int8" in name_lower:
        return "int8"
    # Check prequant before nf4 (more specific match first)

...
# ---------------------------------------------------------------------------
# INT4 PATCH: Pre-quantized NF4 checkpoint support
# Allows loading checkpoints that were already quantized with export_nf4.py,
# skipping the on-the-fly quantization step for faster startup.
# ---------------------------------------------------------------------------
...
def _convert_linear_layers_to_bnb4(
    ...
    """
    Replace every nn.Linear inside *module* with a bitsandbytes Linear4bit (NF4).
    Used when bnb_mode='nf4-prequant' to prepare the model structure before
    loading a pre-quantized state dict.
    """
    try:
        import bitsandbytes as bnb
...
def _load_prequantized_bnb4_state_dict(
    ...
    """
    Load a pre-quantized NF4 state dict into a model that has already been
    converted with _convert_linear_layers_to_bnb4().
    Returns the set of model parameter keys successfully loaded from the
    quantized state dict so the caller can ignore those in strict checks.
    """
    try:
        import bitsandbytes as bnb
    ...
    consumed_model_keys: set = set()
    consumed_state_keys: set = set()
...
class BaseTransformer(nn.Module):
    ...
    def from_pretrained(
        ...
        if bnb_mode == "int8":
            ...
            # INT4 PATCH: Load a pre-quantized NF4 checkpoint (created with tools/export_nf4.py).
            # Faster startup than on-the-fly nf4: no quantization step needed.
            logger.info(
            ...
            # weights dict still holds the full state dict at this point;
            # _load_prequantized_bnb4_state_dict will consume the quantized keys from it.
